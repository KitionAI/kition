...
downloads the exact runtime asset from the pinned `KitionAI/kition` GitHub
...
- prerelease tags such as `v1.0.0-beta.3` or `v1.1.0-rc.1` use the `rc`
...
`Prepare Unified Release` validates the package/runtime lock, creates a draft
Release in `KitionAI/kition`, and dispatches the private runtime workflow.
...
the existing draft Release. Its upload credential must be a GitHub App or
fine-grained token restricted to Release contents in `KitionAI/kition`.

`Publish Unified Release` verifies those assets, prepares the runtime locally,
packages the macOS and Windows clients without private source access, uploads
the desktop artifacts to the same Release, and publishes it.
