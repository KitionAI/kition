---
version: alpha
name: Kition-design-system
description: Kition uses a confident AI workspace identity with project purple (#5645d4), white and light-neutral product surfaces, restrained borders, and real product imagery. Inter is the canonical typeface. Product UI favors compact, work-focused layouts with 8px buttons and 12px cards.
...
Kition product UI adopts **Direction A: Quiet Workspace** as the current
implementation baseline. For app surfaces such as workspace shell, document
editor, Table, Settings, Workflow, Scenario, and Agent drawers, follow
`docs/product-ui-style.md` before applying the broader marketing-oriented
analysis below.

The short version: keep product surfaces white or light neutral, use
`#5645d4` only for primary actions and selected states, keep ordinary buttons
at 8px radius, product cards/dialog panels at 12px radius, and prefer quiet
hairline borders plus light shadows over decorative treatment.

### Marketing vs product scope

This document is the **marketing-page baseline**: hero bands, pricing tables,
feature tile palettes, FAQ accordions, footer regions. The `typography`,
`colors`, and `components:` blocks below describe that marketing surface.
Treat them as references for site-wide brand consistency, not as the source
of truth for app interior surfaces.

The **product UI source of truth** is `docs/product-ui-style.md`. It owns the
token set, dark-mode behaviour, responsive rules, and component norms for
every surface inside the workspace shell. When a rule here disagrees with
`product-ui-style.md`, the product file wins for in-app components.

### Product UI Appendix (pointers)

The product file documents these surface families in detail; this appendix
exists so that anyone landing on `docs/design.md` first can quickly find the
correct section instead of borrowing a marketing token:

| Product surface | Owner doc + section |
|---|---|
| Workspace shell, sidebar, topbar, tabs | `docs/product-ui-style.md` → Workspace Shell, Topbar And Tabs |
| Toolbars, icon buttons, primary CTA placement | `docs/product-ui-style.md` → Toolbars |
| Table grid, table headers, search field, popovers | `docs/product-ui-style.md` → Tables |
| Markdown table widget (in-document table editing) | `docs/product-ui-style.md` → Markdown Table Widget |
| Settings rows, panes, async feedback | `docs/product-ui-style.md` → Settings |
| Agent / Scenario drawers, close behaviour, AI states | `docs/product-ui-style.md` → Agent And Scenario Drawers |
| Workflow canvas, nodes, zoom controls, AI entry | `docs/product-ui-style.md` → Workflow |
| Minimum desktop width and narrow-viewport collapse | `docs/product-ui-style.md` → Responsive And Small Windows |
| CSS variable mapping (light + dark) | `docs/product-ui-style.md` → CSS Variable Mapping, Dark Mode |

Implementation tokens (light and dark) live in `src/app/styles.css`. Do
not hardcode hex values in product components; read from the CSS variables
documented in the product file.

## Overview

Kition presents a coherent workspace where documents, tables, browser research, workflows, and AI tools share one product surface. Marketing pages should lead with tangible product capabilities and show real Kition workspace captures rather than relying on abstract positioning.

Supporting sections should demonstrate real document editing, structured data, visual workflows, and Agent actions. Use the pastel feature palette to distinguish capabilities while keeping the main product surfaces white or light neutral.

The system uses Inter across every UI surface. Buttons are `{rounded.md}` (8px) rectangles, not pills. Cards use `{rounded.lg}` (12px) consistently.

**Key Characteristics:**
- Project purple ({colors.primary}) for the dominant action
- Real Kition workspace imagery that reveals the product clearly
- Bold yellow feature banner ({colors.card-tint-yellow-bold}) for high-emphasis content sections
- Pastel feature card palette (peach, rose, mint, lavender, sky, yellow) echoing the live product table fields
- Inter across every UI surface
- 8px-rounded buttons (NOT pills), 12px-rounded cards — sober editorial geometry
- Dense, scan-friendly product layouts

## Colors

> These tokens define the public Kition brand and must remain consistent with the product UI guidance above.

### Brand & Primary
- **Kition Purple** ({colors.primary}): Signature primary CTA color. Reserved for the dominant action.
...
### Brand Color Spectrum (echoes live product table fields)
...
- **Charcoal** ({colors.charcoal}): Body emphasis (Kition's signature warm-charcoal)
...
> **Marketing baseline.** Marketing and product surfaces use the Inter stack
> defined in `docs/product-ui-style.md` under Typography.

### Font Family
**Inter** (primary): Fallbacks are -apple-system, system-ui, 'Segoe UI', Helvetica, and sans-serif.

### Hierarchy

| Token | Size | Weight | Line Height | Letter Spacing | Use |
|---|---|---|---|---|---|
| `{typography.hero-display}` | 80px | 600 | 1.05 | -2px | Product-name hero |
...
Kition's geometry is sober-editorial — `{rounded.md}` (8px) buttons distinguish it from pill-button-everywhere brands.
...
**`workspace-mockup-card`** — Embedded Kition workspace UI mockup showing real onboarding content.
- Background `{colors.canvas}`, rounded `{rounded.lg}`, border `1px solid {colors.hairline}`, deep shadow `rgba(15, 15, 15, 0.20) 0px 24px 48px -8px`. Carries actual Kition product UI mock.
...
- Left: Kition "N" logo + "Product / AI / Solutions / Resources / Enterprise / Pricing / Request a demo" links.
- Right: "Download Kition" purple button + "Log in" link.
...
- 6-column link grid (Product / Download / Resources / Use cases / Company / Legal).
...
- Apply `{rounded.md}` (8px) to buttons consistently — Kition uses rectangles, not pills
...
- Don't use pill-shaped buttons; Kition's geometry is rectangular-sober
...
| Desktop | 1024 – 1279px | Four-column comparison row. Hero 72px. |
...
3. Run `pnpm run build:check` after implementation changes
...
- Marketing dark-mode token values are not surfaced beyond hero bands in this
  file. Product (in-app) dark mode is owned by `docs/product-ui-style.md` →
  Dark Mode and `src/app/styles.css` (`.dark` selector); refer to those
