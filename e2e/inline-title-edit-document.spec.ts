...
test('pasted image stays rendered beside the cursor and while its source is open', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('.document-editor .cm-content')).toBeVisible({ timeout: 30_000 })
  ...
  const zoomButton = page.locator('.cm-md-image-zoom')
  await expect(zoomButton).toBeVisible()
  await zoomButton.click()
  await expect(page.getByTestId('document-image-preview-overlay')).toBeVisible()
  await expect(page.locator('.document-image-preview-stage img')).toBeVisible()
  await page.getByTestId('document-image-preview-close').click()
  await expect(page.getByTestId('document-image-preview-overlay')).toHaveCount(0)

  await image.click({ button: 'right' })
  await expect(page.getByRole('menuitem', { name: 'Copy image' })).toBeVisible()
  await expect(page.getByRole('menuitem', { name: 'Remove image' })).toBeVisible()
  await expect(page.getByRole('menuitem', { name: 'Reset size' })).toHaveAttribute('aria-disabled', 'true')
  await bodyLine.click()
