...
const getCacheKey = (width: number, height: number, userSets: IUserCell['data']) => {
  return `${String(width)}-${String(height)}-${userSets.map((u) => u.id).join(',')}`;
};
...
export const userCellRenderer: IInternalCellRenderer<IUserCell> = {
  ...
  measure: (cell: IUserCell, props: ICellMeasureProps) => {
    const { data: userSets, readonly, contentAlign = 'left' } = cell;
    ...
    const contentTop =
      userSets.length === 1
        ? getCenteredBlockTop(height, ITEM_HEIGHT, CELL_VERTICAL_PADDING)
    ...
    const cacheKey = getCacheKey(width, height, userSets);
    ...
    for (let i = 0; i < userSets.length; i++) {
      ...
      if (!readonly) {
        const deleteX = (userSets.length === 1 ? x : rectX) + chipWidth - cellHorizontalPadding - fontSizeXS;
        positions.push({
  ...
  checkRegion: (cell: IUserCell, props: ICellClickProps, shouldCalculate?: boolean) => {
    ...
    const { width, height, isActive, hoverCellPosition, activeCellBound } = props;

    const [hoverX, hoverY] = hoverCellPosition;
    const cacheKey = getCacheKey(width, height, data);
