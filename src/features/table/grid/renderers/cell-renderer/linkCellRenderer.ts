...
const computeTextPositions = ({
  ...
  const pendingPositions: Array<Omit<ITextPosition, 'y'>> = [];
  let index = 0;
  ...
  for (const text of data) {
    ...
    for (const { text: lineText, width: textWidth } of textLines) {
      if (pendingPositions.length >= maxLines) break;

      const drawX = getAlignedContentStart(
  ...
  const blockTop = centerVertically
    ? getCenteredBlockTop(
        height,
        getTextBlockHeight(pendingPositions.length, fontSizeSM, cellTextLineHeight),
        cellVerticalPaddingMD,
      )
    : cellVerticalPaddingMD;

  return pendingPositions.map((position, row) => ({
    ...position,
    y: y + blockTop + row * cellTextLineHeight,
  }));
};
...
export const linkCellRenderer: IInternalCellRenderer<ILinkCell> = {
  ...
  checkRegion: (cell: ILinkCell, props: ICellClickProps, _shouldCalculate?: boolean) => {
    ...
    const textPositions = computeTextPositions({
      ...
      theme,
      isActive,
      contentAlign: cell.contentAlign,
    });
