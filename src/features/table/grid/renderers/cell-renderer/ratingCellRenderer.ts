...
import { getAlignedContentStart } from './alignment';
...
export const ratingCellRenderer: IInternalCellRenderer<IRatingCell> = {
  ...
  draw: (cell: IRatingCell, props: ICellRenderProps) => {
    const { data, icon, color, max, readonly, contentAlign = 'left' } = cell;
    ...
    const contentWidth = max * iconSizeXS + (max - 1) * gapSize;
    let currentX = getAlignedContentStart(
    ...
    const contentStartX = currentX - x;
    const maxHoverX = contentStartX + contentWidth;
  ...
  checkRegion: (cell: IRatingCell, props: ICellClickProps, shouldCalculate?: boolean) => {
    const { data, max, readonly, contentAlign = 'left' } = cell;
    ...
    const contentWidth = max * iconSizeXS + (max - 1) * gapSize;
    const minX = getAlignedContentStart(
    ...
    if (inRange(x, minX, maxX) && inRange(y, height / 2 - iconSizeXS, height / 2 + iconSizeXS)) {
      ...
      const newData = Math.ceil((x - minX) / (iconSizeXS + gapSize));
      return {
