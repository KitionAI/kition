...
export const selectCellRenderer: IInternalCellRenderer<ISelectCell> = {
  ...
  measure: (cell: ISelectCell, props: ICellMeasureProps) => {
    const { displayData, readonly, showAddButton, contentAlign = 'left' } = cell;
    ...
    const contentTop =
      displayData.length === 1
        ? getCenteredBlockTop(height, iconSizeSM, SELECT_CELL_PADDING_TOP)
        : SELECT_CELL_PADDING_TOP;
    ...
    const cacheKey = `${String(width)}-${String(height)}-${showAddButton ? 'add-' : ''}${displayData.join(',')}`;
    ...
    for (const text of displayData) {
      ...
      if (displayData.length === 1 && lineCount === 1) {
        x = getAlignedContentStart(
          baseX,
          drawArea.width,
          addBtnOffset + actualOptionWidth,
          contentAlign,
        ) + addBtnOffset;
      }
  ...
  draw: (cell: ISelectCell, props: ICellRenderProps) => {
    ...
    const { displayData, choiceMap, readonly, showAddButton, contentAlign = 'left' } = cell;
    ...
    if (isActive && showAddButton && !readonly && displayData.length === 0) {
      spriteManager.drawSprite(ctx, {
        sprite: GridInnerIcon.Add,
        x: baseX,
        y: drawArea.y + (iconSizeSM - addBtnSize) / 2,
        size: addBtnSize,
        theme,
    ...
    for (const text of displayData) {
      ...
      if (displayData.length === 1 && row === 1) {
        const rowStartX = getAlignedContentStart(
          baseX,
          drawArea.width,
          addBtnOffset + actualOptionWidth,
          contentAlign,
        );
        x = rowStartX + addBtnOffset;
      ...
      } else if (isActive && showAddButton && !readonly && row === 1 && x === firstRowX) {
        spriteManager.drawSprite(ctx, {
          sprite: GridInnerIcon.Add,
          x: baseX,
          y: drawArea.y + (iconSizeSM - addBtnSize) / 2,
  ...
  checkRegion: (cell: ISelectCell, props: ICellClickProps, shouldCalculate?: boolean) => {
    ...
    const { width, height, isActive, hoverCellPosition, activeCellBound, theme } = props;
    ...
    if (showAddButton) {
      ...
      const addBtnY =
        displayData.length <= 1
          ? getCenteredBlockTop(height, theme.iconSizeSM, SELECT_CELL_PADDING_TOP)
          : SELECT_CELL_PADDING_TOP;
