...
export const imageCellRenderer: IInternalCellRenderer<IImageCell> = {
  ...
  draw: (cell: IImageCell, props: ICellRenderProps) => {
    ...
    const imageItems = data.map((imageItem) => {
      const image = imageManager.loadOrGetImage(imageItem.url, columnIndex, rowIndex);
      return {
        imageItem,
        image,
    ...
    const totalImageWidth = imageItems.reduce(
      (total, item, index) => total + item.width + (index > 0 ? INNER_PADDING : 0),
      0,
    );
    const contentOrigin = x + cellHorizontalPadding + initPadding;
    const contentWidth = width - cellHorizontalPadding * 2 - initPadding;
    let drawX = getAlignedContentStart(
      contentOrigin,
      contentWidth,
      totalImageWidth,
      contentAlign,
    ...
    for (const { imageItem, image: img, width: imgWidth } of imageItems) {
