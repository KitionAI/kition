import { contractColorForTheme } from '../../_shim/grid-core';
...
export const drawCells = (
  ...
  const { coordInstance, theme, shouldRerender, scrollState, visibleRegion } = props;
  const { fontFamily, fontSizeSM, fontSizeXS, cellLineColor } = theme;
  const { rowInitSize, freezeRegionWidth, containerWidth, containerHeight, freezeColumnCount } =
    coordInstance;
...
const drawAppendRowColumnDividers = (
  ...
  if (startColumnIndex > stopColumnIndex) return;
  for (const appendRow of appendRowList) {
    const { y, height, coordInstance } = appendRow;
    for (let columnIndex = startColumnIndex; columnIndex <= stopColumnIndex; columnIndex++) {
      const x = coordInstance.getColumnRelativeOffset(columnIndex, scrollLeft);
      const columnWidth = coordInstance.getColumnWidth(columnIndex);
      drawLine(ctx, {
...
export const drawFreezeRegionDivider = (
  ...
  const { scrollLeft, scrollTop } = scrollState;
  const { freezeRegionWidth, containerHeight, totalHeight } = coordInstance;
  const isTop = dividerRegion === DividerRegion.Top;

  const startY = isTop ? 0 : containerHeight;
  // Cap the top divider at the content bottom. On a sparse grid (few rows, no
  // vertical overflow) totalHeight < containerHeight, so this stops the frozen
  // column's line at the append row instead of trailing full-height into empty
  // canvas. When rows overflow, totalHeight >= containerHeight and it's a no-op.
  const endY = isTop ? Math.min(containerHeight, totalHeight - scrollTop) : height;
