...
import { LoadingIndicator, ErrorIndicator, AIOverlayIndicator } from './components';
import type { IAIOverlayCell } from './components';
import type { IGridTheme } from './configs';
import { gridTheme, GRID_DEFAULT, DEFAULT_SCROLL_STATE, DEFAULT_MOUSE_STATE } from './configs';
import { useResizeObserver } from './hooks';
import { useRafState } from './hooks/useReactRuntime';
...
export interface IGridExternalProps {
  ...
  onUndo?: () => void;
  onRedo?: () => void;
  getClipboardText?: (selection: CombinedSelection) => string;
  ...
  /**
   * Cells that have ai_config.enabled — rendered with a hover regenerate button
   * and a spinner overlay when their cell key is in `pending` state in `aiCellStore`.
   */
  aiOverlayCells?: IAIOverlayCell[];
  onCellAIAction?: (cellItem: ICellItem) => void;
}
...
const GridBase: ForwardRefRenderFunction<IGridRef, IGridProps> = (props, forwardRef) => {
  ...
  const groupRowsInfo = useMemo(() => {
    ...
    groupPoints.forEach((point) => {
      ...
      if (type === LinearRowType.Row) {
        ...
        const isInCollapsedGroup = collapsedDepth < Number.MAX_VALUE;
        ...
        if (!isInCollapsedGroup && hasAppendRow) {
  ...
  const getLinearRow = useCallback(
    (index: number): ILinearRow => {
      ...
      return (linearRows[index] ?? { realIndex: -2 }) as ILinearRow;
    },
  ...
  return (
    <div className="size-full" style={style} ref={ref}>
      ...
      {aiOverlayCells && aiOverlayCells.length > 0 && onCellAIAction && (
        <AIOverlayIndicator
          cells={aiOverlayCells}
          coordInstance={coordInstance}
          scrollState={scrollState}
          real2RowIndex={real2RowIndex}
          onAction={onCellAIAction}
        />
      )}
    </div>
  );
