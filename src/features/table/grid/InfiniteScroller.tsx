...
import { useIsTouchDevice } from './_shim/use-is-touch-device';
...
export function getHorizontalScrollerGeometry(
  containerWidth: number,
  scrollWidth: number,
  contentInset: number
) {
  ...
  const safeContentInset = Math.max(0, contentInset);

  return {
    left: 0,
    viewportWidth: safeContainerWidth,
    contentWidth: safeScrollWidth + safeContentInset,
  };
}
...
const InfiniteScrollerBase: ForwardRefRenderFunction<ScrollerRef, ScrollerProps> = (props, ref) => {
  ...
  // The row-control gutter is part of the scroll range, but it should not indent
  // the visible horizontal scrollbar. Adding it to the placeholder width keeps
  // the previous maximum scrollLeft while letting the rail span the full grid.
  const horizontalGeometry = getHorizontalScrollerGeometry(containerWidth, scrollWidth, left);

  return (
    <>
      <div
        ref={horizontalScrollRef}
        data-t-grid-horizontal-scroller
        className={cn(
        ...
        style={{
          ...
          width: horizontalGeometry.viewportWidth,
        }}
      ...
      >
        <div
          className="absolute"
          style={{
            width: horizontalGeometry.contentWidth,
            height: 1,
          }}
        />
      </div>
