...
>(
  ...
  useEffect(() => {
    ...
    const targetElement = (element?.current ?? window) as HTMLElement | Window | MediaQueryList;
...
export const useDismissableLayer = <T extends HTMLElement = HTMLElement>(
  ref: React.RefObject<T | null>,
  open: boolean,
  onDismiss: () => void,
  mouseEvent: 'mousedown' | 'mouseup' = 'mousedown'
): void => {
  useOnClickOutside(ref, open ? () => onDismiss() : undefined, mouseEvent);
