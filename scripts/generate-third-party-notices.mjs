...
const assetNoticesPath = path.join(repositoryDir, 'docs/legal/ASSET_NOTICES.txt')
...
function renderNotices(inventory, installedPackageIndex) {
  ...
  const assetNotices = fs.existsSync(assetNoticesPath)
    ? fs.readFileSync(assetNoticesPath, 'utf8').trim()
    : ''
  ...
  return [
    ...
    '',
    assetNotices || 'No separately licensed bundled assets.',
    '',
    divider,
    '',
    'Package Inventory',
